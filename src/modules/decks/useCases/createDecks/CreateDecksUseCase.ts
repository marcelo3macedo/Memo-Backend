...
export class CreateDecksUseCase {
  ...
  async execute({ name, parentId, userId, isPublic, clonedBy }: ICreateDecksDTO): Promise<Deck> {
    return await this.decksRepository.create({ name, parentId, userId, isPublic, clonedBy });
  }
}